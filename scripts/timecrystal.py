...
class Bose:
  ...
  def __init__( self, a, b, npt, init, g, G, rot, P, dt, **kwargs ):
    ...
    self.V                = mex_hat ( self.x, self.y, self.P, 3. )
...
################################################################################
  ...
  def converge( self            ,
    ...
    Returns true if <H-chmpot> = 0.
    Verbose will print <H-chmpot>
    H is the system Hamiltonian, chmpot is the chemical potential
    '''
    
    enList = self.energies( verbose = False )
    chmpot = enList[0] + 2 * enList[1] + enList[3] + enList[4]
             #The Self-Interaction must be doubled, as this is <H>, not the
             #total energy
    check = chmpot - sum( sum( self.psi.conjugate() * chmpot * self.psi() ) )
    return abs( check ) < 1e-9
