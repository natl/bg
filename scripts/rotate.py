...
class Bose:
  ...
  def __init__( self, a, b, npt, init, g, G, rot, P, dt, **kwargs ):
    ...
    self.psi = np.zeros( [self.npt, self.npt], dtype = complex )
  ...
  def fftwsetup( self ,
    ...
    forw = fftw3.Plan( self.rspc, self.fspc, direction='forward', 
                       flags=['estimate'], nthreads=nthreads )
    back = fftw3.Plan( self.rspc, self.fspc, direction='backward', 
                       flags=['estimate'], nthreads=nthreads )
  ...
  def gravity( self ):
    ...
    return self.G * self.dx * self.dy * (
           ff.fftshift( ff.ifft2( ff.fft2( ff.fftshift( den ) ) * self.flog ) )
                                        )
  ...
  def stepRK4(self):
    ...
    k4 = self.dt * nonlin.dot( self.intpic( self.psi ).reshape( n2 ) 
                             ).reshape( sq )
  ...
  def intpic( self, field ):
    ...
    '''
    return ff.fftshift( ff.ifft2( self.expksquare * 
                         ff.fft2( ff.fftshift( field ) ) ) )
