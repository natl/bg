...
class Bose:
  ...
  def __init__( self, a, b, npt, init, g, G, rot, P, dt, **kwargs ):
    ...
    self.rpsi, self.fpsi, self.pfft, self.pifft = self.fftwsetup()
    print id(self.psi)
    self.rden, self.fden, self.denfft, self.denifft = self.fftwsetup()
    ...
    self.expksquare       = np.exp( 1.j * 0.5 * self.dt * self.ksquare )
    self.log              = np.log( self.rad )
    self.flog             = abs( ff.fft2( ff.fftshift( -self.log ) ) )
  ...
  def fftwsetup( self ,
                 nthreads = 2):
    '''
    rspc, fspc, forw, back = fftwsetup( self , nthreads = 2)
    
    rspc = real space array
    fspc = fourier space array
    forw = FFT
    back = IFFT
    Plan some FFTs, then return them to a higher level function, as well
    as the correct arrays
    '''
    rspc = np.zeros( [self.npt, self.npt], dtype = complex )
    fspc = np.zeros( [self.npt, self.npt], dtype = complex )
    forw = fftw3.Plan( rspc, fspc, direction='forward', 
                       flags=['estimate'], nthreads=nthreads )
    back = fftw3.Plan( rspc, fspc, direction='backward', 
                       flags=['estimate'], nthreads=nthreads )
    return rspc, fspc, forw, back
  ...
  def normalise( self ):
    ...
    return is None
    '''
    self.psi /= np.sqrt( sum( sum ( abs( self.psi ) ** 2. ) )
             * self.dx * self.dy )
    return None
  #-----------------------------------------------------------------------------
  def k_vector( self ):
  ...
  def gravity( self ):
    ...
    '''
    self.rden[:] = abs(self.psi)**2.  #calculate the probability density
    
    #convolve
    self.rden[:] = fftshift( self.rden[:] )
    self.denfft()
    self.fden *= self.flog
    self.denifft()
    ...
    return self.G * self.dx * self.dy * fftshift( self.den )
  ...
  #-----------------------------------------------------------------------------
  def intpic( self, field ):
    '''
    intpic( self, field ):
    transform field from real space to Interaction Picture
    '''
    self.rpsi[:] = fftshift( field[:] )
    self.pfft()
    self.fpsi *= self.expksquare
    self.pifft()
    
    return ff.fftshift( self.rpsi[:] )
